...
$total_students  = (new Student())->count();
$total_classes   = (new ClassRoom())->count();
$total_teachers  = (new User())->countTeachers();
$total_sessions  = $attendanceModel->getTotalSessions();

$recent_sessions = $attendanceModel->getRecentSessions('admin', $_SESSION['user_id']);
$atRisk          = $attendanceModel->getAtRiskStudents('admin', $_SESSION['user_id']);
$atRiskCount     = count($atRisk);
$sectionRates = $attendanceModel->getSectionAttendanceRates();

$sectionLabels = []; $sectionData = []; $sectionColors = [];
foreach($sectionRates as $c){
    $sectionLabels[] = $c['label'];
    $sectionData[]   = (float)$c['rate'];
    $rate = (float)$c['rate'];
    $sectionColors[] = $rate >= 90 ? '#28a745' : ($rate >= 75 ? '#f0ad4e' : '#d9534f');
}
?>
<!DOCTYPE html>
<html lang="en">
        ...
        <div class="content-section">
            ...
            <div class="breakdown-bar-graph">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                    <h3 class="section-heading" style="margin-bottom:0;">Attendance Rate per Section</h3>
                    ...
                    <tbody>
                        <?php if($recent_sessions->num_rows > 0): ?>
                            ...
                            <tr>
                                ...
                                <td><span class="badge <?php echo $s['status']; ?>"><?php echo strtoupper($s['status']); ?></span></td>
...
<script>
    ...
    const trendChart = new Chart(document.getElementById('trendChart'), {
        type: 'line',
        data: { labels: [], datasets: [{ label: 'Avg Attendance Rate', data: [], borderColor: '#2c3e50', backgroundColor: 'rgba(44,62,80,0.1)', tension: 0.4, fill: true, pointBackgroundColor: '#2c3e50', spanGaps: true }] },
    ...
    const allClassLabels = <?php echo json_encode($sectionLabels); ?>;
    const allClassData   = <?php echo json_encode($sectionData); ?>;
    const allClassColors = <?php echo json_encode($sectionColors); ?>;
    const PAGE_SIZE = 4;
    let classPage = 0;
